...
和委托推送会立即唤醒主循环，最终状态仍以券商同步查询结果为准；没有推送时
每1秒查询一次，撤单确认阶段每0.5秒查询一次。从配置的第一次下单时间F
（默认09:30:42）起，最多5分钟内每5秒复核新鲜买一：价格不变则
...
15:30；11:30至13:00只等待。委托和成交推送立即唤醒订单生命周期，缺少推送
时每1秒同步查询，撤单确认每0.5秒查询。任一时刻最多允许一笔本策略委托处于
非终态。
...
通过前会拒绝启用。故障通知是可选旁路，不属于启用门禁。实盘任务不启用“错过后立即补跑”，
因此收盘后启用只影响下一个计划时点。
...
两个执行器在进入 `HALTED` 后发送通知（微信推送WxPusher和/或邮件），不把
通知发送结果作为交易状态
转移条件：

- 未配置通知时执行器照常运行，`rr on`也不会因此失败；
- 配置缺失、损坏、令牌/密码解密失败或通道不可用时只记录本地警告；
- 通知发送前，`HALTED` 和失败原因必须已经原子写入日志；
- 通知失败不会重开状态机、补单、撤单或改变退出码；
- 每个通道最多进行3次有界重试，每次连接超时10秒；
- 同一日志中的同一失败键发送成功后，重启不会重复发送；
- 通知只包含策略、环境、状态、原因、未决委托标志和本机日志路径，
  不包含证券账号、SMTP密码或WxPusher令牌。

配置命令：

```powershell
.\rr wx   # 微信推送（推荐，扫码一次取得SPT令牌）
.\rr mail
```

微信SPT令牌与SMTP密码使用Windows当前用户DPAPI加密，分别保存在被忽略的
`config/repo_failure_wxpusher_secret.local.clixml`与
`config/repo_failure_email_secret.local.clixml`；非机密配置保存在
`config/repo_failure_wxpusher.local.json`与
